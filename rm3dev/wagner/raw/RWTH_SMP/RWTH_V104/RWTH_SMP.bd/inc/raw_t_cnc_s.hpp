...
//---------------------------------------------------------------------------
   ...
   #define AG_MOD_FLEX                 5   // AW-Verschleißminimierend fahrendes Axialgeruest
...
//---------------------------------------------------------------------------
   struct Def_Maschine_Motor
   {
      ...
      float ueberlastung_p;         // Motorüberlastung           [%]
   ...
   { 
      ...
      float anteil_bis_fa_nenn_proz;   // Anteil an Mantellänge für Fa_nenn               [%]
      float anteil_bis_fa_hoch_proz;   // Anteil an Mantellänge für Fa_hoch               [%]
      ...
      float sigma_b_max_zul_welle;  // Zulässige Biegespannung                   [N/mm²] (ehemals N/cm²)
      ...
      float a_kolben;                  // Fläche Kolbenseite                              [mm²]
      float a_ring;                    // Fläche Ringseite                                 [mm²]
   ...
   { 
      ...
      float f_kommi_hoch;                // gesetzte kommissionierte Kraft bei Hochdruck      [N]
      float f_kommi_nenn;                // gesetzte kommissionierte Kraft bei Nenndruck      [N]
      float kraftniveau_ueber_BE;        // Abstand über BE falls Zweischlittenmaschine gewählt
      float kraftniveau_unter_BE;        // Abstand unter BE falls Zweischlittenmaschine gewählt
      float l_dw_unter_BE;               // Länge DW unter BE (je nach ws_dw_einspannung bis Lager oder Lagermitte)
      float l_dw_ueber_BE;               // Länge DW unter BE (je nach ws_dw_einspannung bis Lager oder Lagermitte)
      
                                         // Absenkbares Dornlager
      float l_max_dw_ueber_BE;           // Max.Länge DW ueber BE (je nach ws_dw_einspannung bis Lager oder Lagermitte)
      float l_min_dw_ueber_BE;           // Min.Länge DW ueber BE (je nach ws_dw_einspannung bis Lager oder Lagermitte)
      
      int   f_max_manu_flag;            // Kraft direkt vorgeben (ohne Berechung mit Systemdruck)
      int   hoch_druck_flag;            // Nenn- oder Hochdruckanwendung
      int   zwei_radialschlitten_flag;   // Ein- oder Zweischlittenmaschine
      int   ws_dw_einspannung;         // Schalter für Art der DW-Einspannung (FA, ESE, BSE)

      // resultierende Grössen
      float a_kolben_o;                  // Fläche Kolbenseite oben            [mm²]
      float a_kolben_u;                  // Fläche Kolbenseite unten         [mm²]
      float a_ring_o;                  // Fläche Ringseite oben            [mm²]
      float a_ring_u;                  // Fläche Ringseite unten            [mm²]
      ...
      float l_dw_gesamt;               // Gesamte Dornlänge f(ws_dw_einspannung, l_dw_unter_BE, l_dw_ueber_BE)
   };
   ...
   { 
      ...
      // resultierende Grössen
      float a_kolben;                  // Fläche Kolbenseite               [cm²]
      float a_ring;                     // Fläche Ringseite                  [cm²]
   ...
   struct Def_dPos_Dorn_oben
   {
      ...
      float d_rwg_ou_korr;          // Soll Änderung RWG oben zu unten     [mm/s]    

      float deltas_soll_o;          // Soll Abnahme oben                   [mm]
      float deltas_soll_u;          // Soll Abnahme unten                  [mm]
      float d_deltas_ou_soll;       // Soll Änderung oben zu unten         [mm]

      float dalpha_dreh_ou_soll_bo; // Soll Winkeländerung Dornwalze [rad]
...
//---------------------------------------------------------------------------
   struct Def_AW_Geo
   {
      ...
      float l_fehlende_spitze;      // Länge der fehlenden Spitze                [mm]
      float l_nutz;                 // Nutzbare Mantellänge                      [mm]
                                    // Theor. Mantellänge - Länge fehlende Spitze
      float l_zyl_ansatz;           // Länge des zylindrischen Ansatz            [mm]
      float winkel_gr;              // Öffnungswinkel der unteren Axialwalze     [°]

      // resultierende Grössen
      float delta_w_konstr_bo;      // Delta Winkel oben   (Bogenmaß)            [bog]
      float du_grund;               // Grunddurchmesser                          [mm]
      float du_spitze;              // Durchmesser Spitze                        [mm]
      float l_mantel_theo;          // Theor. Mantellänge                        [mm]
      float winkel_bo;              // Öffnungswinkel der Axialwalze             [bog]

      float achsenlaenge_l_fehlende_spitze;  // Länge der fehlenden Spitze bez.
                                             // auf Achse                        [mm]
      float achsenlaenge_l_nutz;    // Nutzbare Länge bez. auf Achse             [mm]
...
//---------------------------------------------------------------------------
   struct Def_Schlupf
   {
      float b_weg_ag;      // Beschleunigungsweg Axialgerüst                        [mm]
...
//---------------------------------------------------------------------------
   ...
   {
      // Oben
      float ram_o;               // Radius gedrückte Länge oben                           [mm]
      float raa_o;                                                                        
      float rai_o;                                                                        
      ...
      // Unten
      float ram_u;               // Radius gedrückte Länge unten                           [mm]      
      float raa_u;                                                                        
      float rai_u;                                                                        
...
//---------------------------------------------------------------------------
   struct Def_Ibf_nominal
   {
      ...
      int   Zen_mode;       //Betiebsart Positionsreg. (0) oder Verdränger (1)
...
//---------------------------------------------------------------------------
   struct Def_Ibf_actual
   {
      ...
      float Ist_AG;         //Messachse Axialgerüst (HW-Aussen - Spitze AW)[mm]
