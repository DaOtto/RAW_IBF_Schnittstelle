...
RWTH_EXCH::RWTH_EXCH(void)
{
    ...
    //int i; //derzeit nur Hilfsfunktion für INIFileTest -> definition der IBF_PARA
    ...
    Soll_Inter.ag_mode = 0; // Axialgerüst fährt beim Anfahren auf die SMS vorgegebene Position
    Soll_Inter.ag_act  = 1;
    ...
    // Öffnen der IBF_DATA um vorgegebene Werte einzulesen
    CIniFile iniFile( "IBF_Data.ini");
         if(!iniFile.ReadFile()) //Fehlerbehandlung
           ...
           {
            MyLogFile << "----------------------"<< endl;
            MyLogFile << "IBF_Data wurde geöffnet"<< endl;
    ...
    MyLogFile << "Minimale Ringhöge                                                   " << Soll_IBF.min_ASO_Pos << endl;
    MyLogFile << endl;
    MyLogFile << endl;
