...
/*-----------------------------------------------------------------------------------------*/
/*          Includedatein die für die IBF-Strukturen benötoigt werden                      */
/*-----------------------------------------------------------------------------------------*/
...
/*-----------------------------------------------------------------------------------------*/
    ...
    struct IBF_Flag
    {
        ...
        int AG_Start_erreicht;      /* Flag: Startposition Axialgerüst erreicht = 1*/
        int Bissverhaeltnis;        /* Flag: Walzen mit Bissverhältnis (1)|| ohne Bissverhältnis (0)*/

        int DA_Jenkouk_DW_Regelung; /* Flag zum zuschalten/abschalten der Dornwalzenreglung aus der DA Jenkouk 2008*/
                                    /* benötigt derzeit den temporären Messaufbau */
        int DA_Jenkouk_Messen_ohne_Fahren; /*nur temporär für Einrichtebetrieb -> danach wieder löschen !!!!!!!!*/
...
/*------------------------------------------------------------------------------*/
    ...
    /*Positionen Merker*/
        ...
        float ASO_Pos_Null;     /* Vorförmhöhe aus Carwin                 [mm]*/
        ...
        float ASO_Pos_Differenz;    /* Ist_IBF.ASO_Pos_Differenz = Korektur der Starthöhe aus Carwin (in IBF_ini definiert) */
                                    /* ASO wird sonst auf Starthöhe gesetzt und beim 'Positonieren des AG rutscht ASO über  */
                                    /* Stirnseite                                                                       [mm]*/
     ...
     /*Geometrie*/
       ...
       float Rohling_h_angefahren;      /* Rohlingshöhe nach dem Anfahren       [mm] */
       float Rohling_s_angefahren;      /* Rohlingswanddicke nach dem Anfahren  [mm] */
       float da;                        /* aktueller Außendurchmesser           [mm] */
    ...
    float delta_tr;                 /* Tastrollendifferenz: Zum Positionieren des Axialgerüstes beim Start [mm]*/
    ...
    struct stuetzstellen_var stuetz_var;            // Struktur Stützstellenberechnung SA Sascha Frank, Mai 2008
    struct Def_DW_Regelung_DA_Jenkouk DW_Regelung;  // Struktur Dornwalzenregelung für das axiale Profilieren DA Jenkouk April-Juli 2008
    struct Axialgeruest_Regelung AG_Regelung;       // Struktur Dornwalzenregelung für das axiale Profilieren DA Jenkouk April-Juli 2008
...
//---------------------------------------------------------------------------
    ...
    float tr_pos;           /* Sollposition für Tastrolle beim Axialprofilieren [mm]*/
    float delta_tr;                 /* Tastrollendifferenz: Zum Positionieren des Axialgerüstes beim Start [mm]*/
   ...
   /* Flexibel Radial Profliert */
    ...
    struct Radial_Flex_Prof Radial;
    struct Axial_Flex_Prof Axial;
    struct stuetzstellen_var stuetz_var;   // Struktur Stützstellenberechnung
