...
//   #define T1_KW_KRAFT           5.0F //6.0F 21.10.2011 Morandini Koppers Debye
//   #define T1_KW_MOMENT          5.0F //6.0F
   ...
   struct Def_Ein_Puff
   {
      ...
      int      power_taster_flag;               // Flag, Powertaster gedrueckt [0,1]   
      ...
      int      aps_1_walzung_flag;              // Flag, 1. Walzung Axialprofil Walzen [0,1]
      ...
      int      f_axial_begrenzung_flag;         // Axiale Kraftbegrenzung in CARWIN gesetzt [0,1]
      int      f_axial_begrenzung_aktiv;        // Axiale Kraftbegrenzung/Regler aktiv [0,1]
      ...
      // Laserposition fuer Grunddrehzahl Axialwalzen
      ...
      struct   Def_Form_Mess   form_mess_mw_vol_of; // Aktuelle Messwerte Mittelwert Volumen ungefiltert
      float    da_id;
      ...
      float    T1_KW_KRAFT;                     // Von aussen vorgebbar: Glaettungszeit KW Kraft [s], Standard 5.0
      float    T1_KW_MOMENT;                    // Von aussen vorgebbar: Glaettungszeit KW Moment [s], Standard 5.0
      ...
      struct   Def_Schlupf    schlupf_u;        // Schlupfgroessen
      ...
      // Axialprofilwalzen
      int      Dorn_Messung_flag;                 // Dorn ist unten, Wanddicke gueltig [0,1]              
      ...
      struct Def_EIN_RMS rms;

      // Koppers Filter
      double Anteil_RU_Filterung;   // Standard 0.5 wird nicht mehr gebraucht
      double Anteil_SF_korr;        // Standard 1.0 wird nicht mehr gebraucht

      // Reserve
      double   Anteil_MF_t_dkorr; //  wird nicht mehr gebraucht
      double   dis_hw_dw_ohne_federkennlinie;     // Abstand zwischen Hauptwalze - Dornwalze   [mm]
      double   dis_aw_ohne_federkennlinie;        // Abstand zwischen den Axialwalzen          [mm]
      double   dummy3;
      double   dummy4;
      double   dummy5;
      double   dummy6;
...
//---------------------------------------------------------------------------
   struct Def_Aus_Puff
   {
      ...
      double   y_Bezug_Z_mf;
      struct   Def_Form_Mess    form_mess_mw_vol_mf;  // Aktuelle Messwerte Mittelwert Volumen gefiltert
      float    vorform_hoehe_variabel;                // sich durch radiale Breitung aendernde Hoehe
      int      di_vorhanden_flag;                     // Flag ob Innendurchmesser vorhanden

      struct   Def_Ufz_Rad       ufz_rad_of;          // Groessen Umformzone radial ohne Filter
      struct   Def_Ufz_Axi       ufz_axi_of;          // Groessen Umformzone axial ohne Filter

      struct   Def_Ufz_Rad      ufz_rad_mf;           // Groessen Umformzone radial mit Filter
      struct   Def_Ufz_Axi      ufz_axi_mf;           // Groessen Umformzone axial mit Filter
      ...
      float    wp_aw_n_pos_ra_1a_ms_hori_o;
      ...
      //float    dpm_mf_stark_geglaettet;         // Ringwachsgeschw. Bezug Dm geglaettet   [mm/s] 
      float    dpm_of;                          // Ringwachsgeschw. Bezug Dm nicht geglaettet [mm/s]
      ...
      float    pos_laser_aw_n_mit_spitze_o_mf;
      struct   Def_n_omega            n_omega;

      float    hw_r_dv0_wp;                     // Geschwindigkeitsradius der Hauptwalze     [mm]
      float    wp_r_dv0_hw;
      float    aw_r_dv0_u;
      float    aw_r_dv0_o;
      float    wp_r_dv0_aw_u;
...
//---------------------------------------------------------------------------
   struct Def_Lokal_Puff
   {
      ...
      float    fr_of_kw;                        // Radiale Kraft ungefiltert fuer KW-Berechnung [N]

      float    fa_min;                          // Minimum Kraft axial     [N]
      float    fa_of_kw;                        // Axiale Kraft ungefiltert fuer KW-Berechnung [N]
      ...
      float    mr_of_kw;                        // Radiales Moment ungefiltert fuer KW-Berechnung [Nmm]

      float    ma_of;                           // Axiales Moment ungefiltert   [Nmm]
      float    ma_min;                          // Minimum Moment axial         [Nmm]
      float    ma_of_kw;                        // Axiales Moment ungefiltert fuer KW-Berechnung [Nmm]
      ...
      float      kw_min;                        // Minimum Umformwiderstand [N/mm2]
      float      kw_max;                        // Maximum Umformwiderstand [N/mm2]
   ...
   //---------------------------------------------------------------------------
   // Struktur Def_Ein_Puff_V7
   //---------------------------------------------------------------------------
   struct Def_Ein_Puff_V7
   {
      // Wahlschalter
      int      abschalt_modus;                  // Abschaltmodus [Aussen,Mitte,Innen]
      int      endphase_flag;                   // Flag, Endphase [0,1]
...
//---------------------------------------------------------------------------
// Struktur Def_Aus_Puff_V7
//---------------------------------------------------------------------------
   struct Def_Aus_Puff_V7
   {
      float    pos_rs_achse;       
      float    pos_rs_achse_start; 
      float    pos_wp_achse;       
...
//---------------------------------------------------------------------------
// Struktur Def_Lokal_Puff_V7
//---------------------------------------------------------------------------
   struct Def_Lokal_Puff_V7
   {
      // Kraefte
      float    fr_min;                          // Minimum Kraft radial    [N]
      float    fr_of_kw;                        // Radiale Kraft ungefiltert fuer KW-Berechnung [N]
