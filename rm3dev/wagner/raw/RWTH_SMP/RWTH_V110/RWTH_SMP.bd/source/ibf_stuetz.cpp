	...
	if (Soll_IBF->stuetz_var.crtimer <= 100)   // Teilumdrehung gelaufen. Stützstellenwechsel und neue Berechnung					
	{
		...
		for (int i_for = 0; i_for < Soll_IBF->stuetz_var.skip_faktor; i_for++)
		{
			...
			{	//falls Stellen übersprungen werden müssen werden diese hier mit Mittelwerten aufgefüllt
				...
				{
					Soll_IBF->stuetz_var.stellen[0][Soll_IBF->stuetz_var.merker] = (float)((Soll_IBF->stuetz_var.
																							stellen[0][(Soll_IBF->
																										stuetz_var.
																										merker -
																										i_for - 1 +
																										maxstellen)] +
																							Ist_IBF.RS_Pos) / 2);
					if (Soll_IBF->stuetz_var.stellen[0][(Soll_IBF->stuetz_var.merker - i_for - 1 + maxstellen)] == 0)
						Soll_IBF->stuetz_var.stellen[0][Soll_IBF->stuetz_var.merker] = Ist_IBF.RS_Pos;
					Soll_IBF->stuetz_var.stellen[1][Soll_IBF->stuetz_var.merker] = (float)((Soll_IBF->stuetz_var.
																							stellen[1][(Soll_IBF->
																										stuetz_var.
																										merker -
																										i_for - 1 +
																										maxstellen)] +
																							Ist_IBF.ASO_Pos) / 2);
					if (Soll_IBF->stuetz_var.stellen[1][(Soll_IBF->stuetz_var.merker - i_for - 1 + maxstellen)] == 0)
						Soll_IBF->stuetz_var.stellen[1][Soll_IBF->stuetz_var.merker] = Ist_IBF.ASO_Pos;
				}
				else
				{
					Soll_IBF->stuetz_var.stellen[0][Soll_IBF->stuetz_var.merker] = (float)((Soll_IBF->stuetz_var.
			...
			if (Soll_IBF->stuetz_var.merker >= maxstellen)
				Soll_IBF->stuetz_var.merker = 0;
		}
		//if (Soll_IBF->stuetz_var.merker >= maxstellen) Soll_IBF->stuetz_var.merker = 0;						
	}
	else
		Soll_IBF->stuetz_var.crtimer = Soll_IBF->stuetz_var.crtimer - 100; //Timer herunterzählen. Programm wird alle 100ms aufgerufen.			



	Soll_IBF->stuetz_var.kontrollmerker = Soll_IBF->stuetz_var.kontrollmerker + (float)(maxstellen / (10 *
																									  Ist_Inter.
																									  Werkzeug.
																									  Ringumdrehung));
					 ...
					 struct Def_IBF_Allgemein_SOLL* Soll_IBF)
{
	...
	if ((Soll_IBF->stuetz_var.stellen[0][maxstellen - 1] > 0) && (Soll_IBF->stuetz_var.crtimer <= 100))	//Ausführung nur, falls komplettes Array bereits gefüllt und timer Ausführung vorsieht!
	{
		...
		dHstrich = (float)(Ist_Inter.Werkzeug.Durchmesser_HW * Ist_Inter.da / (Ist_Inter.Werkzeug.Durchmesser_HW +
																			   Ist_Inter.da));
		...
		dDstrich = (float)(d_innen * Ist_Inter.Werkzeug.Durchmesser_DW / (d_innen -
																		  Ist_Inter.Werkzeug.Durchmesser_DW));
		...
		if (Ist_IBF.stuetz_var.cr <= 0.001)	//Cr muss vorgegeben sein!
		{
			...
			Soll_IBF->stuetz_var.delta_s = (float)(2 * rrstrich / pow(Soll_IBF->stuetz_var.cr,
												   2) + sqrt(8 * S0r * rrstrich / pow(Soll_IBF->stuetz_var.cr,
												   2) + 4 * pow(rrstrich, 2) / pow(Soll_IBF->stuetz_var.cr, 4)));
		}

			//Soll_IBF->RS_Pos = delta_S -S0r = angestrebte Wanddicke = Ziel dieser Berechnung!!! delta_S = (cr*smr)²/fiktiver Radius
		else
			Soll_IBF->stuetz_var.delta_s = (float)(2 * rrstrich / pow(Ist_IBF.stuetz_var.cr,
												   2) + sqrt(8 * S0r * rrstrich / pow(Ist_IBF.stuetz_var.cr,
												   2) + 4 * pow(rrstrich, 2) / pow(Ist_IBF.stuetz_var.cr, 4)));
					...
					struct Def_IBF_Allgemein_SOLL* Soll_IBF)
{
	...
	if ((Soll_IBF->stuetz_var.stellen[1][maxstellen - 1] > 0) && (Soll_IBF->stuetz_var.crtimer <= 100))	//Ausführung nur, falls komplettes Array bereits gefüllt und timer Ausführung vorsieht!
	{
		...
		{
			Soll_IBF->stuetz_var.ca = (float)(sqrt((h0a - h1a) * tan(22.5 * PI / 180) * (axi_Rm - axi_e))) / hma;
			Soll_IBF->stuetz_var.delta_h = (float)(pow(Soll_IBF->stuetz_var.ca, 2) * pow(hma,
												   2) / (tan(22.5 * PI / 180) * (axi_Rm - axi_e)));
		}
			//Soll_IBF->ASO_Pos = delta_h -h0r = angestrebte axiale Ringhöhe = Ziel dieser Berechnung!!!
		else
			Soll_IBF->stuetz_var.delta_h = (float)(pow(Ist_IBF.stuetz_var.ca, 2) * pow(hma,
												   2) / (tan(22.5 * PI / 180) * (axi_Rm - axi_e)));
	}
