...
#include <fstream>
   ...
   /* Stream zu einer Logdatei, zum Debugen der main(...) */
...
RWTH_EXCH::RWTH_EXCH(void)
{
    ...
    Soll_IBF.Zeit.t_Halte= 20;    // Sollwartezeit beim Anfahren
    ...
    if(Ist_IBF.Flag.Bissverhaeltnis == 1)
        {
        Soll_IBF.Axial.AGR_Pr[0] = Ist_IBF.Axial.AGR_Pr[0]; // Notwendig wenn mehrere Biss gewalzt werden, damit die AG nach Positionierung per Biss nicht auf ersten Wert 'zurückfährt'
...
int RWTH_EXCH::Main_Control(void)
{
   ...
   if(running_flag && RwwtPool->Data_initialed && RwwtPool->Data_available)
   {
    ...
    //jeweils zum Walzenstart (Walzenstartflag) müssen einige Variablen vordefiniert werden, damit richtige CASE-Anweisungen im rpw.cpp (IBF-Modul) ausgeführt werden
      ...
      if(Ist_IBF.Flag.Axial_Radialwalzen == 0)                       /* !!! Darf nur aktiv sein, wenn Axialprofilieren angewählt wird und Kragenwalze verbaut ist !!! */
      {
      aus_onli.fa_max_axialwalze = 400000.0F;                        /* Kraftgrenzenregler für Kragenwalze auf 40T, ansonsten wird Kraft anhang Kegelwalze auf mittlerer Wanddicke begrenzt */
      }
