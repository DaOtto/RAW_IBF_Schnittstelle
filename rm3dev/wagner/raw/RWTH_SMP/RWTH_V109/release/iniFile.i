#line 1 "C:\\rm3dev\\wagner\\raw\\RWTH_SMP\\RWTH_V109\\RWTH_SMP.bd\\source\\iniFile.cpp"
...
#line 26 "C:\\rm3dev\\wagner\\raw\\RWTH_SMP\\RWTH_V109\\RWTH_SMP.bd\\inc\\ibf_tool.hpp"
...
#line 25 "C:\\rm3dev\\wagner\\raw\\RWTH_SMP\\RWTH_V109\\RWTH_SMP.bd\\source\\iniFile.cpp"
...
bool CIniFile::ReadFile()
{
  ...
  while( getline( f, line)) {
    ...
    if ( line.length()) {
      ...
      if ( !( __chattr((char)((line[0]) & 0xFF)) & (0x0080) )) {
...
bool CIniFile::WriteFile()
{
  ...
  for ( commentID = 0; commentID < comments.size(); ++commentID)
    wf << ' ; ' << comments[commentID] << '\r' << endl; 
  if ( comments.size())
    wf << '\r' << endl;


  for ( keyID = 0; keyID < keys.size(); ++keyID) {
    wf << '[' << names[keyID] << ']' << '\r' << endl;

    for ( commentID = 0; commentID < keys[keyID].comments.size(); ++commentID)
      wf << ';' << keys[keyID].comments[commentID] << '\r' << endl;

    for ( valueID = 0; valueID < keys[keyID].names.size(); ++valueID)
      wf << keys[keyID].names[valueID] << '=' << keys[keyID].values[valueID] << '\r' << endl;
    wf << '\r' << endl;
  }
  wf.close();
